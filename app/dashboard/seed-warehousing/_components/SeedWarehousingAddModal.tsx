...
import { createSeedWarehousing } from "@/app/lib/services/seedWarehousing";
...
export default function SeedWarehousingAddModal({
    open,
    onCancel,
    seedPackages,
    warehouses,
    ...
    const handleFinish = async (values: any) => {
        ...
        try {
            ...
            const result = await createSeedWarehousing(payload);

            if (result.success) {
                setMessage({ status: "ok", message: "تراکنش انبار با موفقیت ثبت شد" });
                setTimeout(() => {
                    onSuccess();
                    onCancel();
                }, 1000);
    ...
    return (
        ...
        >
            {/* Header */}
            <div className="relative px-6 py-6 bg-gradient-to-br from-emerald-50 via-lime-50/80 to-white dark:from-emerald-950 dark:via-emerald-900/50 dark:to-slate-900 border-b border-emerald-100 dark:border-emerald-800/50">
                <button
                    onClick={handleClose}
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="font-bold text-2xl text-emerald-900 dark:text-emerald-100">ثبت تراکنش جدید</h3>
                        <p className="text-sm text-emerald-600/80 dark:text-emerald-400/80 mt-1 flex items-center gap-1.5">
                            <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
                            اطلاعات ورود و خروج بذر را وارد کنید
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="px-6 py-6 bg-gradient-to-br from-white to-slate-50/30 dark:from-slate-900 dark:to-slate-950">
                <Form form={form} layout="vertical" onFinish={handleFinish} initialValues={{ TransactionType: true }}>
                    ...
                    <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 mt-8 pt-6 border-t-2 border-slate-200 dark:border-slate-700">
                        ...
                        />
                        <GreenhouseButton
                            text={loading ? "در حال ثبت..." : "ثبت تراکنش"}
                            variant="primary"
                            type="submit"
                            loading={loading}
