...
export default function NurseryRoomsEditModal({ open, setOpen, setLoading, data, refreshData }: NurseryRoomsEditModalProps) {
    ...
    useEffect(() => {
        ...
        // Cleanup happens naturally, NO form.resetFields() here to avoid warning
    }, [open, data]);
    ...
    const inputStyleClass = "rounded-xl border-2 border-slate-200 dark:border-slate-700 hover:border-emerald-300 dark:hover:border-emerald-700 focus:border-emerald-400 dark:focus:border-emerald-600 transition-all shadow-sm hover:shadow w-full disabled:bg-gray-50 disabled:text-gray-500 disabled:border-gray-200 dark:bg-slate-800 dark:text-white dark:placeholder-slate-500";
        ...
        >
            ...
            <div className="relative px-6 py-6 bg-gradient-to-br from-emerald-50 via-lime-50/80 to-white dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 border-b border-emerald-100 dark:border-slate-700">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="font-bold text-2xl text-emerald-900 dark:text-slate-100">ویرایش اتاق نشاء</h3>
                        <p className="text-sm text-emerald-600/80 dark:text-slate-400 mt-1 flex items-center gap-1.5">
                            <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
                            ویرایش اطلاعات: {data?.NurseryRoomName}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="px-6 py-6 bg-gradient-to-br from-white to-slate-50/30 dark:from-slate-900 dark:to-slate-950">
                <Form form={form} layout="vertical" onFinish={handleFinish} requiredMark={false}>
                    ...
                    <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 mt-8 pt-6 border-t-2 border-slate-200 dark:border-slate-700">
                        ...
                        />
                        <GreenhouseButton
                            text={internalLoading ? "در حال ثبت..." : "ویرایش اتاق نشاء"}
                            variant="primary"
                            type="submit"
