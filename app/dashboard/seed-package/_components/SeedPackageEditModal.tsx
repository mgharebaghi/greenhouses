...
import { updateSeedPackage } from "@/app/lib/services/seedPackage";
...
interface SeedPackageEditModalProps {
    open: boolean;
...
export default function SeedPackageEditModal({ open, setOpen, setLoading, data }: SeedPackageEditModalProps) {
    ...
    const fetchBatches = async () => {
        const res: any = await getAllSeedBatches();
        if (res) {
            setBatches(res.map((b: any) => ({
                value: b.SeedBatchID,
                label: `کد: ${b.BatchCode} - ${b.CropVariety ? b.PlantVarities?.VarietyName : 'نامشخص'}`
            })));
        }
    };
    ...
    const handleFinish = async (values: any) => {
        ...
        // Date conversion
        if (values.PackagingDate instanceof DateObject) {
            values.PackagingDate = values.PackagingDate.toDate();
        }

        const res = await updateSeedPackage({ id: data.SeedPackageID, data: values });
        ...
        >
            ...
            <div className="relative px-6 py-6 bg-gradient-to-br from-blue-50 via-indigo-50/80 to-white dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 border-b border-blue-100 dark:border-slate-700">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="font-bold text-2xl text-blue-900 dark:text-slate-100">ویرایش بسته بذر</h3>
                        <p className="text-sm text-blue-600/80 dark:text-slate-400 mt-1 flex items-center gap-1.5">
                            <span className="h-1.5 w-1.5 rounded-full bg-blue-400 animate-pulse"></span>
                            اطلاعات بسته بندی را ویرایش کنید
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="px-6 py-6 bg-gradient-to-br from-white to-slate-50/30 dark:from-slate-900 dark:to-slate-950">
                <Form form={form} layout="vertical" onFinish={handleFinish} requiredMark={false}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <Form.Item
                            name="SeedBatchID"
                            label={<span className="text-sm font-semibold text-slate-700 dark:text-slate-300">انتخاب بچ کد مرجع <span className="text-rose-500">*</span></span>}
                            rules={[{ required: true, message: 'لطفا بچ کد را انتخاب کنید' }]}
                            className="mb-0"
                        >
                            <Select
                                options={batches}
                        ...
                        <Form.Item
                            name="SerialNumber"
                            label={<span className="text-sm font-semibold text-slate-700 dark:text-slate-300">شماره سریال بسته <span className="text-rose-500">*</span></span>}
                        ...
                        <Form.Item name="PackageType" label={<span className="text-sm font-semibold text-slate-700 dark:text-slate-300">نوع بسته‌بندی</span>} className="mb-0">
                        ...
                        <Form.Item name="Status" label={<span className="text-sm font-semibold text-slate-700 dark:text-slate-300">وضعیت</span>} className="mb-0">
                            <Input placeholder="مثال: موجود, فروخته شده..." size="large" className={inputStyleClass} style={inputStyle} />
                        </Form.Item>

                        <Form.Item name="IsCertified" label={<span className="text-sm font-semibold text-slate-700 dark:text-slate-300">گواهی شده؟</span>} valuePropName="checked" className="mb-0 sm:col-span-2">
                            <Switch />
                        </Form.Item>
                    ...
                    <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 mt-8 pt-6 border-t-2 border-slate-200 dark:border-slate-700">
                        ...
                        />
                        <GreenhouseButton
                            text={internalLoading ? "در حال ثبت..." : "ویرایش بسته بذر"}
                            variant="primary"
                            type="submit"
