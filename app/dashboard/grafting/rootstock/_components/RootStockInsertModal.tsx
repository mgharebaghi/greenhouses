...
import { CloseOutlined, ExperimentOutlined, CheckCircleOutlined, ExclamationCircleOutlined, PlusOutlined } from "@ant-design/icons";
import { createRootStockPlant } from "@/app/lib/services/grafting/rootstock/create";
import DatePicker from "react-multi-date-picker";
import DateObject from "react-date-object";
import persian from "react-date-object/calendars/persian";
import persian_fa from "react-date-object/locales/persian_fa";

interface RootStockInsertModalProps {
    open: boolean;
...
export default function RootStockInsertModal({
    open,
    setOpen,
    setLoading,
    refreshData,
    options
}: RootStockInsertModalProps) {
    ...
    const onFinish = async (values: any) => {
        ...
        const res = await createRootStockPlant(values);

        setLoading(false);
        setInternalLoading(false);

        if (res.success) {
            setMessage({ status: "ok", message: "گیاه پایه جدید با موفقیت ثبت شد" });
            refreshData();
            setTimeout(() => {
                setOpen(false);
            }, 1000);
        ...
        >
            ...
            <div className="relative px-6 py-6 bg-gradient-to-br from-emerald-50 via-lime-50/80 to-white dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 border-b border-emerald-100 dark:border-slate-700">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="font-bold text-2xl text-emerald-900 dark:text-slate-100">ثبت گیاه پایه جدید</h3>
                        <p className="text-sm text-emerald-600/80 dark:text-slate-400 mt-1">
                            اطلاعات مربوط به گیاه پایه
                        </p>
                    </div>
                </div>
            </div>
