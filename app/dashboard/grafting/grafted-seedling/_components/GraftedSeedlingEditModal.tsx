...
interface GraftedSeedlingEditModalProps {
    open: boolean;
...
export default function GraftedSeedlingEditModal({
    open,
    setOpen,
    data,
    refreshData,
    options
}: GraftedSeedlingEditModalProps) {
    ...
    const onFinish = async (values: any) => {
        ...
        try {
            const payload = {
                ...values,
                ReadyForSaleDate: selectedDate ? new Date(selectedDate).toISOString() : null
            };

            const result = await updateGraftedSeedling(data.GraftedPlantID, payload);
    ...
    return (
        <ConfigProvider direction="rtl">
            ...
            >
                ...
                <div className="relative px-6 py-6 bg-gradient-to-br from-orange-50 via-amber-50/80 to-white dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 border-b border-orange-100 dark:border-slate-700">
                    <div className="flex items-center gap-4">
                        <div className="h-12 w-12 rounded-xl bg-orange-100/50 dark:bg-orange-900/30 flex items-center justify-center text-orange-600 dark:text-orange-500 border border-orange-200/60 dark:border-orange-700/60">
                            <EditOutlined className="text-2xl" />
                        </div>
                        <div>
                            <h3 className="font-bold text-xl text-slate-800 dark:text-slate-100">ویرایش نشاء پیوندی</h3>
                            <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">ویرایش اطلاعات و مشخصات رکورد #{data?.GraftedPlantID}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-8 bg-white dark:bg-slate-900">
                    <Form form={form} layout="vertical" onFinish={onFinish} className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
