...
import { updateGraftingOperation } from "@/app/lib/services/grafting/operation/update";
import { CloseOutlined, ExperimentOutlined, CheckCircleOutlined, ExclamationCircleOutlined } from "@ant-design/icons";
import GreenhouseButton from "@/app/components/UI/GreenhouseButton";

interface GraftingOperationEditModalProps {
    open: boolean;
...
export default function GraftingOperationEditModal({
    open,
    setOpen,
    setLoading,
    refreshData,
    data,
    options
}: GraftingOperationEditModalProps) {
    ...
    useEffect(() => {
        if (open && data) {
            form.setFieldsValue({
                ...
                // Ensure number fields are set correctly if null
                GraftedNumber: data.GraftedNumber,
                SucceedGrafted: data.SucceedGrafted,
                SuccessRate: data.SuccessRate,
                RecoveryPeriodDays: data.RecoveryPeriodDays
            });
    ...
    const handleFinish = async (values: any) => {
        ...
        try {
            const result = await updateGraftingOperation(data.GraftingID, values);
            ...
            setMessageState({ status: "error", message: "خطا در ویرایش اطلاعات" });
        } finally {
        ...
        >
            ...
            <div className="relative px-6 py-6 bg-gradient-to-br from-emerald-50 via-lime-50/80 to-white dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 border-b border-emerald-100 dark:border-slate-700">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="font-bold text-2xl text-emerald-900 dark:text-slate-100">ویرایش عملیات پیوند #{data?.GraftingID}</h3>
            ...
            <div className="px-6 py-6 bg-gradient-to-br from-white to-slate-50/30 dark:from-slate-900 dark:to-slate-950">
                <Form form={form} layout="vertical" onFinish={handleFinish} requiredMark={false}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <Form.Item
                            name="NurserySeedID"
                            label={<span className="text-sm font-semibold text-slate-700 dark:text-slate-300">پیوندک (نشاء) <span className="text-rose-500">*</span></span>}
                        ...
                        <Form.Item
                            name="SuccessRate"
                            label={<span className="text-sm font-semibold text-slate-700 dark:text-slate-300">نرخ موفقیت (%)</span>}
                            className="mb-0"
                        >
                            <InputNumber
                                className={inputStyleClass}
                        ...
                        <Form.Item
                            name="FinalResult"
                            label={<span className="text-sm font-semibold text-slate-700 dark:text-slate-300">نتیجه نهایی</span>}
                            className="mb-0"
                        >
                            <Select
                                placeholder="وضعیت نهایی"
                                options={[
                                    { label: 'موفق', value: 'Success' },
                                    { label: 'ناموفق', value: 'Failure' },
                                    { label: 'نامشخص', value: 'Unknown' },
                        ...
                        <Form.Item
                            name="RecoveryPeriodDays"
                            label={<span className="text-sm font-semibold text-slate-700 dark:text-slate-300">دوره نقاهت (روز)</span>}
                            className="mb-0"
                        >
                            <InputNumber
                                className={inputStyleClass}
                    ...
                    <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 mt-8 pt-6 border-t-2 border-slate-200 dark:border-slate-700">
                        ...
                        />
                        <GreenhouseButton
                            text={internalLoading ? "در حال ذخیره..." : "ذخیره تغییرات"}
                            variant="primary"
                            type="submit"
