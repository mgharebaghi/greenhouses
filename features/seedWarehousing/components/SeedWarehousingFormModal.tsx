...
import { createSeedWarehousing, updateSeedWarehousing } from "@/features/seedWarehousing/services";
import { CloseOutlined, ExperimentOutlined, EditOutlined, CheckCircleOutlined, ExclamationCircleOutlined } from "@ant-design/icons";
import GreenhouseButton from "@/shared/components/GreenhouseButton";

interface SeedWarehousingFormModalProps {
    open: boolean;
    onCancel: () => void;
    data?: any | null;
    seedPackages: any[];
...
export default function SeedWarehousingFormModal({
    open,
    onCancel,
    data,
    seedPackages,
        ...
        try {
            const payload = {
                ...
                TransactionDate: values.TransactionDate instanceof DateObject
                    ...
                    : (isEdit ? values.TransactionDate : new Date()),
                PackageQuantity: Number(values.PackageQuantity),
            };

            let result;
            if (isEdit) {
                result = await updateSeedWarehousing(data.ID, payload);
            } else {
                result = await createSeedWarehousing(payload);
            }

            if (result.success) {
                setMessage({ status: "ok", message: isEdit ? "تراکنش با موفقیت ویرایش شد" : "تراکنش با موفقیت ثبت شد" });
                setTimeout(() => {
                    onSuccess();
                    onCancel();
                }, 1000);
            ...
            setMessage({ status: "error", message: "خطایی رخ داد" });
        } finally {
            setLoading(false);
        }
    };
    ...
    const theme = isEdit ? {
        ...
        dotBg: "bg-blue-400",
        title: "ویرایش تراکنش",
        subtitle: "ویرایش اطلاعات تراکنش انبار",
    } : {
        gradient: "from-emerald-50 via-lime-50/80 to-white dark:from-emerald-950 dark:via-emerald-900/50 dark:to-slate-900",
        iconBg: "from-emerald-500 via-emerald-600 to-emerald-700",
        dotBg: "bg-lime-400",
        title: "ثبت تراکنش جدید",
        subtitle: "اطلاعات ورود و خروج بذر را وارد کنید",
    };
    ...
    return (
        ...
        >
            {/* Header */}
            <div className={`relative px-6 py-6 bg-gradient-to-br ${theme.gradient} border-b border-emerald-100 dark:border-slate-700`}>
                <button
                    onClick={handleClose}
                    className="absolute top-5 left-5 h-9 w-9 rounded-xl bg-white dark:bg-slate-800 hover:bg-emerald-50 dark:hover:bg-emerald-900/50 border border-emerald-200 dark:border-emerald-700/50 hover:border-emerald-300 transition-all flex items-center justify-center text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 shadow-sm hover:shadow"
                    aria-label="بستن"
                    type="button"
                >
                ...
                <div className="flex items-center gap-4">
                    <div className="relative">
                        ...
                        <div className={`absolute -bottom-1 -right-1 h-5 w-5 rounded-full ${theme.dotBg} border-2 border-white dark:border-slate-800`}></div>
                    </div>
                    <div>
                        <h3 className="font-bold text-2xl text-emerald-900 dark:text-emerald-100">{theme.title}</h3>
                        <p className="text-sm text-emerald-600/80 dark:text-emerald-400/80 mt-1 flex items-center gap-1.5">
            ...
            <div className="px-6 py-6 bg-gradient-to-br from-white to-slate-50/30 dark:from-slate-900 dark:to-slate-950">
                <Form form={form} layout="vertical" onFinish={handleFinish} initialValues={{ TransactionType: true }}>
                    ...
                    {message && (
                        <div
                            className={`mt-5 p-4 rounded-xl border-2 flex items-start gap-3 animate-in fade-in slide-in-from-top-3 duration-300 shadow-sm ${message.status === "ok"
                                ? "bg-gradient-to-br from-emerald-50 to-emerald-100/50 dark:from-emerald-900/40 dark:to-emerald-800/20 border-emerald-300 dark:border-emerald-700 text-emerald-900 dark:text-emerald-100"
                                : "bg-gradient-to-br from-rose-50 to-rose-100/50 dark:from-rose-900/40 dark:to-rose-800/20 border-rose-300 dark:border-rose-700 text-rose-900 dark:text-rose-100"
                                }`}
                        >
                            <div className={`mt-0.5 p-1.5 rounded-lg ${message.status === "ok" ? "bg-emerald-200/50 dark:bg-emerald-500/20" : "bg-rose-200/50 dark:bg-rose-500/20"}`}>
                                {message.status === "ok" ? (
                    ...
                    <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 mt-8 pt-6 border-t-2 border-slate-200 dark:border-slate-700">
                        ...
                        />
                        <GreenhouseButton
                            text={loading ? (isEdit ? "در حال ذخیره..." : "در حال ثبت...") : (isEdit ? "ذخیره تغییرات" : "ثبت تراکنش")}
                            variant="primary"
                            type="submit"
                            loading={loading}
