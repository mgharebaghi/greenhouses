...
function isAndroidTablet(): boolean {
  ...
  const hasTabletViewport =
    Math.min(window.screen.width, window.screen.height) >= 600;
  const reportsAsTablet =
    nav.userAgentData?.mobile === false || !/Mobile/i.test(userAgent);

  return isAndroid && hasTabletViewport && reportsAsTablet;
}

export default function OrdersQRModal({
  open,
  setOpen,
  data,
}: OrdersQRModalProps) {
  ...
  const handleTabletPrintLabel = () => {
    ...
    try {
      ...
      const printableContent = source.cloneNode(true) as HTMLDivElement;
      ...
      sourceCanvases.forEach((canvas, index) => {
        ...
        qrImage.alt = "QR Code";
        qrImage.className = canvas.className;
        qrImage.setAttribute("style", canvas.getAttribute("style") || "");
        qrImage.style.imageRendering = "pixelated";
        clonedCanvas.replaceWith(qrImage);
      });

      const pageSize = labelSize === "auto" ? "auto" : `${labelW} ${labelH}`;
      ...
      printDocument.close();
      printDocument.title = `Order-Label-${fullData?.OrderCode || ""}`;
      printDocument.body.appendChild(
        printDocument.importNode(printableContent, true),
      );
  ...
  const qrFontSize = qrSize === "100%" ? "10px" : `calc(${qrSize} * 0.14)`;
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center p-8 bg-white min-h-[600px]">
        <h3 className="text-xl font-extrabold text-slate-800 mb-8 mt-2">
          گواهی دیجیتال سفارش
        </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-5">
            ...
            <div className="flex flex-col gap-1 border-b border-slate-200 pb-2">
              ...
              <span className="text-sm font-bold text-slate-800">
                {fullData?.OrderDate
                  ? new Date(fullData.OrderDate).toLocaleDateString("fa-IR")
                  : "—"}
              </span>
            </div>
            <div className="flex flex-col gap-1 border-b border-slate-200 pb-2">
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ fontSize: "14px", fontWeight: 900 }}>
                      {fullData?.OrderDate
                        ? new Date(fullData.OrderDate).toLocaleDateString(
                            "fa-IR",
                          )
                        : "—"}
                    </div>
                  ...
                  >
                    <div>
                      ...
                      <div style={{ fontSize: "13px", fontWeight: 700 }}>
                        {fullData
                          ?.Tbl_SeedPackage_Tbl_Orders_RootstockIDToTbl_SeedPackage
                          ?.Tbl_plantVariety?.Tbl_Plants?.CommonName || ""}{" "}
                        -{" "}
                        {fullData
                          ?.Tbl_SeedPackage_Tbl_Orders_RootstockIDToTbl_SeedPackage
                          ?.Tbl_plantVariety?.VarietyName || "—"}
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: "13px", fontWeight: 700 }}>
                        {fullData
                          ...
                          ? fullData
                              .Tbl_SeedPackage_Tbl_Orders_RootstockIDToTbl_SeedPackage
                              .Tbl_suppliers.CompanyName ||
                            `${fullData.Tbl_SeedPackage_Tbl_Orders_RootstockIDToTbl_SeedPackage.Tbl_suppliers.FirstName} ${fullData.Tbl_SeedPackage_Tbl_Orders_RootstockIDToTbl_SeedPackage.Tbl_suppliers.LastName}`
                          : "—"}
                      </div>
                    </div>
                    <div>
                      ...
                      <div style={{ fontSize: "13px", fontWeight: 700 }}>
                        {fullData
                          ...
                          ? fullData
                              .Tbl_SeedPackage_Tbl_Orders_ScionIDToTbl_SeedPackage
                              .Tbl_suppliers.CompanyName ||
                            `${fullData.Tbl_SeedPackage_Tbl_Orders_ScionIDToTbl_SeedPackage.Tbl_suppliers.FirstName} ${fullData.Tbl_SeedPackage_Tbl_Orders_ScionIDToTbl_SeedPackage.Tbl_suppliers.LastName}`
                          : "—"}
                      </div>
                    </div>
                    <div>
              ...
              >
                <div style={{ maxWidth: "55%" }}>
                  ...
                  >
                    این سند برای تایید اصالت سفارش صادر شده است.
                  </div>
                </div>
                <div
                  style={{
            ...
            <div
              style={{
                ...
                fontSize:
                  qrSize === "100%"
                    ? "8pt"
                    : `${Math.max(3.5, Math.min(16, parseInt(qrSize) * 0.32))}pt`,
                whiteSpace: "normal",
                wordBreak: "break-all",
                fontFamily: "tahoma",
              }}
